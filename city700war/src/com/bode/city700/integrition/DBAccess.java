...
public class DBAccess extends Beans
{
	...
	{
		DBAccess dba = null;
		DBAccess dbaMaster = null;
		String strValue = null;
		try
		{
			dbaMaster = new DBAccess();
	...
	public DBAccess() throws Exception
	{
		...
		m_arLogData = null;
		m_nParams = 0;
		m_arChildren = new Vector<DBAccess>();
		m_conn = getConnection();
		m_arReadyStmt = new Vector<ReadyStatement>();
	}

	protected DBAccess(Connection conn, String strSql, boolean bUpdate,
			DBAccess dbaParent) throws Exception
	{
		m_conn = null;
		m_arChildren = null;
		m_bIsChild = false;
	...
	public void commit() throws Throwable
	{
		if (("" == null) && !m_bIsChild)
		{
			for (int i = 0; (m_arChildren != null) && (i < m_arChildren.size()); i++)
			{
				DBAccess child = m_arChildren.elementAt(i);
				if (child.m_bUpdate)
				{
					if (m_stmtPrep == null)
					{
	...
	{
		DBAccess dbaChild = prepareSql(strSql, true);
	...
	public DBAccess prepareSql(String strSql) throws Exception
	{
		return prepareSql(strSql, false);
	}

	public DBAccess prepareSql(String strSql, boolean bUpdate) throws Exception
	{
		DBAccess dbaChild = new DBAccess(m_conn, strSql, bUpdate, this);
